...
This CIP proposes reducing the Celestia block time from 6 seconds to 3 seconds. This is achieved by adjusting the consensus timeout parameters (`TimeoutPropose` and `DelayedPrecommitTimeout`), doubling block-count-based parameters (`MaxAgeNumBlocks` and upgrade height delays) so they continue to represent the same wall-clock durations at the faster block rate, and correcting the IBC `MaxExpectedTimePerBlock` parameter. The `GoalBlockTime` constant is deprecated and removed as it is no longer accurate or necessary.
...
1. **Consensus timeout adjustments**: The following consensus timeout parameters MUST be updated to target a 3-second block time:
    ...
    1. All other timeout parameters (`TimeoutProposeDelta`, `TimeoutPrevote`, `TimeoutPrevoteDelta`, `TimeoutPrecommit`, `TimeoutPrecommitDelta`, `TimeoutCommit`) remain unchanged.
...
| `ibc.ConnectionGenesis.Params.MaxExpectedTimePerBlock` | 75 seconds | 15 seconds | Maximum expected time per block used by IBC connection verification (should be 3-5x block time) | No |
...
**Why not adjust `TimeoutPrevote` and `TimeoutPrecommit`?** These round-based timeout parameters (both currently 3000 ms) are used as fallbacks when the network does not reach consensus in the first round. Keeping them unchanged provides a conservative approach that avoids introducing risk of increased round failures. They may be adjusted in a future CIP if empirical data supports it.
